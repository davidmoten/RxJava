...
public class OperatorMerge2<T> implements Operator<T, Observable<T>> {
    ...
    public Subscriber<? super Observable<T>> call(Subscriber<? super T> child) {
        ...
        Subscriber<Observable<T>> parent = new MergeSubscriber<T>(coordinator);
    ...
    private static class Coordinator<T> {
        ...
        private volatile boolean addingInners = true;
        ...
        public void add(Observable<T> inner) {
        ...
        public synchronized void arrived(T t, long innerExpecting, InnerSubscriber<T> inner) {
            ...
            if (expecting.get() > 0 && innerExpecting == 0) {
                inner.requestMore(expecting.get());
            }
        }
        ...
        public void onCompleted(InnerSubscriber<T> inner) {
            ...
            if (innerSubscribers.size() == 0 && !addingInners) {
    ...
    private static class MergeSubscriber<T> extends Subscriber<Observable<T>> {
        ...
        @Override
        public void onNext(Observable<T> inner) {
    ...
    private static class InnerSubscriber<T> extends Subscriber<T> {
        ...
        public synchronized void requestMore(long n) {
            long more = Math.min(MAX_REQUESTS - expecting.get(), n);
            if (more > 0) {
