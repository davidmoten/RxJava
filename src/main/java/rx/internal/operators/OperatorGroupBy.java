...
public final class OperatorGroupBy<T, K, V> implements Operator<GroupedObservable<K, V>, T> {
    ...
    public Subscriber<? super T> call(Subscriber<? super GroupedObservable<K, V>> t) {
        final GroupBySubscriber<T, K, V> parent = new GroupBySubscriber<T, K, V>(t, keySelector,
                valueSelector, bufferSize, delayError, mapFactory);
    ...
    public static final class GroupBySubscriber<T, K, V> extends Subscriber<T> {
        ...
        final Action1<K> evictionAction;
        ...
        public GroupBySubscriber(Subscriber<? super GroupedObservable<K, V>> actual,
            ...
            if (mapFactory == null) {
                ...
                this.evictionAction = null;
            } else {
                this.evictedKeys = new ConcurrentLinkedQueue<K>();
                this.evictionAction = new Action1<K>() {
                    @Override
                    public void call(K key) {
                        evictedKeys.add(key);
                    }
                };
                this.groups = createMap(mapFactory);
            }
        ...
        private Map<Object, GroupedUnicast<K, V>> createMap(
                Func1<Action1<K>, Map<K, Object>> mapFactory) {
            return (Map<Object, GroupedUnicast<K, V>>) (Map<?, ?>) mapFactory
                    .call(evictionAction);
        }
