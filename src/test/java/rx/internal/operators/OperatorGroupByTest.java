...
public class OperatorGroupByTest {
    ...
    public void mapFactoryThrows() {
        ...
        Observable.just(1)
          .groupBy(UtilityFunctions.<Integer>identity(), UtilityFunctions.identity(), evictingMapFactory)
          .test()
          .assertNoValues()
          .assertError(ex);
    }
    ...
    public void mapFactoryWithExpiringGuavaCacheDemonstrationCodeForUseInJavadoc() {
        ...
        AssertableSubscriber<Integer> ts =
            Observable.range(1, numValues)
                .groupBy(mod5, UtilityFunctions.<Integer>identity(), evictingMapFactory)
                .flatMap(addCompletedKey(completed))
                .test()
                .assertCompleted();
    ...
    private static Func1<GroupedObservable<Integer, Integer>, Observable<? extends Integer>> addCompletedKey(
        ...
        return new Func1<GroupedObservable<Integer, Integer>, Observable<? extends Integer>>() {
            @Override
            public Observable<Integer> call(final GroupedObservable<Integer, Integer> g) {
                return g.doOnCompleted(new Action0() {
                    @Override
                    public void call() {
